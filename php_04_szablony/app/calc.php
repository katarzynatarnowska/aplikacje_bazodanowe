...
function validate(&$form,&$infos,&$msgs,&$hide_intro){
	...
	if ( ! (isset($form['kwota']) && isset($form['raty']) && isset($form['procent']) ))	return false;	
	...
	if ( $form['raty'] == "") $msgs [] = 'Nie podano okresu';
	...
	if ( count($msgs)==0 ) {
		...
		if (! is_numeric( $form['raty'] )) $msgs [] = 'Druga wartość nie jest liczbą';
		if (! is_numeric( $form['procent'] )) $msgs [] = 'Trzecia wartość nie jest liczbą';
	}
...
function process(&$form,&$infos,&$msgs,&$result){
	...
	//wykonanie operacji
	$result = ((($form['procent']/100)*$form['kwota']*$form['raty'])+$form['kwota'])/($form['raty']*12);
}
...
$smarty->assign('page_header','Szablony');
